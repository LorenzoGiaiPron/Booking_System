...
class JobSchedulerThread(threading.Thread):
    ...
    def __init__(self, DailyScheduleURL, dailyEvents):
    ...
    def getDailyTable(self):
        ...
        #
        # TO-DO REST request to database to get the daily schedule
        #
    ...
    def startSchedule(self, Rtime="23:59", url=None):
        if url is not None:
            self.DailyScheduleURL = url # allows to change url
        self.on = True
        schedule.every().day.at(Rtime).do(self.getDailyTable)
        self.start()
...
class MainSystemREST(object):
    ...
    requestScheduler = JobSchedulerThread(DBrequestsURL["daily_schedule"], dailyEvents)
    mqtt = MQTT_MSclient("MainSystem", "test.mosquitto.org")

    # insert a new psw in the daily event schedule
    def newDailyEventPsw(self, eventID, newEventPsw, email):
        global dailyEvents
    ...
    def PUT(self, *uri, **param):
        ...
        if len(uri) != 0:
            ...
            # customer services
            if uri[0] == "customer":
                if uri[1] == "sendPassword":  # return the chosen password
                    ...
                    # format {"name":"username", "mailAddress":"", "timeslot":"", "event_name":"", "event_ID":,"n_tickets":}
                    rawPayload = cherrypy.request.body.read()
                    ReqjsonStr = rawPayload.decode("utf-8")
                    Reqdict = json.loads(ReqjsonStr)
                    # check format of body
                    if "name" in Reqdict.keys() and "mailAddress" in Reqdict.keys() and "event_name" in Reqdict.keys() \
                        ...
                        # TO-DO: send password to database
                        ...
                        raise cherrypy.HTTPError(400, "format of the message body is incorrect")
